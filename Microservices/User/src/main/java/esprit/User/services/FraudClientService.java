...
import esprit.User.dto.FraudAnalyzeRequest;
...
public class FraudClientService {
    ...
    public void analyzeAction(Long userId, String action, String ipAddress, String deviceInfo) {
        try {
            ...
            log.info("Sent fraud analysis for user: {} action: {}", userId, action);
        } catch (Exception e) {
            log.error("Failed to call Fraud Service: {}", e.getMessage());
            // Non-blocking for the actual flow
        }
    }
}
